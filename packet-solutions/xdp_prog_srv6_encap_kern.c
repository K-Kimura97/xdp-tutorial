...
*/
    ...
    if (bpf_xdp_adjust_head(ctx, 0 - (int)(sizeof(*outerip6h) ＋ sizeof(*srh) ＋ sizeof(struct in6_addr))))//(int)sizeof(*srh)) - (int)sizeof(struct in6_addr))
        return -1;
...
*/
	...
	srh = (void *)(outerip6h + 1); //sizeof(struct ipv6hdr);
	if (srh + 1 > data_end)
        return -1;
